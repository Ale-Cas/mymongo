...
ENV VIRTUAL_ENV /opt/mymongo-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/mymongo/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/mymongo/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/mymongo/ && touch src/mymongo/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/mymongo/
